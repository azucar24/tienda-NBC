...
if ($num > 0) {
    if ($_SESSION['sumaTotal'] > 0) {
        ...
        consultasSQL::InsertSQL("venta", "Fecha, Dui, Descuento, TotalPagar, Estado", "'" . date('d-m-Y') . "','" . $nitC . "','0','" . $_SESSION['sumaTotal'] . "','" . $StatusV . "'");

        /* recuperando el número del pedido actual */
        $verId = ejecutarSQL::consultar("select * from venta where Dui='$nitC' order by NumPedido desc limit 1");
        ...
        echo '<br>El pedido se ha realizado con éxito';
    } else {
        echo '<br>No has seleccionado ningún producto, revisa el carrito de compras';
    }
} else {
    echo '<br>El nombre o contraseña invalidos';
}
